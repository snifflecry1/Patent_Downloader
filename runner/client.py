...
from models import PatentRequest, PatentResponse
...
class APIClient:
    ...
    async def fetch_page(
        ...
        params = self.create_payload(
            page=page, size=size, validated_dates=self.validated_dates
        )
    ...
    def ping_initial_metadata(self) -> Optional[PatentResponse]:
        ...
        payload = self.create_payload(
            page=1, size=1, validated_dates=self.validated_dates
        )
        try:
